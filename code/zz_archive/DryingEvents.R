...
metrics_fun <- function(n){
  ...
  df <- read_csv(file = files[n]) %>% 
    mutate(time=as_date(time), 
           num_date = as.numeric(time), 
           q = streamflow_NWIS) %>% 
    select(time, num_date, streamflow_NWIS) %>% 
...
tt <- data.table::rbindlist(x,fill=TRUE)

#Capture finishing time
tf<-Sys.time()
tf-t0

write_csv(tt,paste0('../data/metrics_by_event.csv'))
